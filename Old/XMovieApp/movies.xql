...
declare function local:search_by_years($movies as node()*, $years as xs:string*) as node()*
{
    if (count($years)>0) then 
        let $res :=  $movies/year[$years]/parent::movie
        return
            local:search_by_years($res, subsequence($years, 2))
    else $movies
};

declare function local:search_by_director($movies as node()*, $director as xs:string*) as node()*
{
    if (count($director)>0) then 
        let $res := $movies/director[(lower-case(concat(first_name/text(), ' ', last_name/text()))) = $director]/parent::movie
        return
            local:search_by_director($res, subsequence($director, 2))
    else $movies
};
...
let $action := lower-case(xs:string(request:get-parameter("action", ""))),
    $movies := doc('/db/movies.xml')//movie
